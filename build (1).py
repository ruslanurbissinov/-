...
def remediation_text(inc):
    """Собирает текст рекомендаций + фактически выполненных работ по их устранению."""
    text = inc['recommendation']
    works = inc.get('remediation', [])
    if not works:
        return text
    lines = [text, '', 'Выполненные работы по устранению:']
    for w in works:
...
function render() {{
  ...
  for (var s = 0; s < scored.length; s++) {{
    ...
    htmlStr += '<button class="toggle-btn" type="button">Показать меры и заключение &#9662;</button>';
    htmlStr += '<div class="details">';
    htmlStr += '<p><b>Меры при останове:</b> ' + highlight(inc.measures, tokens) + '</p>';
    htmlStr += '<p><b>Заключение:</b> ' + highlight(inc.conclusion, tokens) + '</p>';
    htmlStr += '<p><b>Принятые меры:</b> ' + highlight(inc.recommendation, tokens) + '</p>';
    if (inc.remediation && inc.remediation.length) {{
      htmlStr += '<p><b>Работы по устранению:</b></p>';
      htmlStr += '<ul style="margin:0 0 8px 18px;padding:0;">';
      for (var w = 0; w < inc.remediation.length; w++) {{
        var wk = inc.remediation[w];
        htmlStr += '<li>' + highlight(wk.work_description || '', tokens);
        htmlStr += ' &mdash; Акт выполненных работ ' + escapeHtml(wk.work_act || '?');
        htmlStr += ' от ' + escapeHtml(wk.work_date || '?');
        var wkLink = wk.work_source_uri || wk.work_source || wk.source;
        if (wkLink) {{
          htmlStr += ' (<a class="doc-link" href="' + wkLink + '" download="' + escapeHtml(wk.work_source || 'work.pdf') + '" target="_blank">открыть акт</a>)';
        }}
        htmlStr += '</li>';
      }}
      htmlStr += '</ul>';
    ...
    if (inc.source_uri) {{
      htmlStr += '<p><b>Документы:</b> <a class="doc-link" href="' + inc.source_uri + '" download="' + escapeHtml(inc.source_name || 'act.pdf') + '" target="_blank">Открыть акт расследования</a></p>';
    }}
    if (inc.defect_source || inc.work_source) {{
      htmlStr += '<p><b>Документы:</b> ';
      var docLinks = [];
      if (inc.defect_source) {{
        docLinks.push('<a class="doc-link" href="' + inc.defect_source + '" download="' + escapeHtml(inc.defect_source_name || 'defect.pdf') + '" target="_blank">Открыть дефектный акт</a>');
      }}
      if (inc.work_source) {{
        docLinks.push('<a class="doc-link" href="' + inc.work_source + '" download="' + escapeHtml(inc.work_source_name || 'work.pdf') + '" target="_blank">Открыть акт выполненных работ</a>');
      }}
      htmlStr += docLinks.join(' &middot; ');
      htmlStr += '</p>';
    }}
    htmlStr += '</div>';
    card.innerHTML = htmlStr;
