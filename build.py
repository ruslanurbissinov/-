...
var I18N = {{
  ru: {{
    ...
    tabInvestigation:'Акт расследования', tabWorkOrder:'Акт выполненных работ',
  ...
  en: {{
    ...
    tabInvestigation:'Investigation report', tabWorkOrder:'Completion report',
...
function initFilters() {{
  var allGpa=[], allCat=[], allYear=[], allType=[];
...
function render() {{
  ...
  if ((fGpa || fCat || fType) && scored.length) {{
    var yrs = [];
...
function renderDashboard() {{
  ...
  var html = '<div class="dash-grid">' +
    ...
    (topGpa ? '<div class="stat-card clickable" data-filter-key="fGpa" data-filter-value="'+escapeHtml(topGpa.label)+'"><div class="num">'+escapeHtml(trGpa(topGpa.label))+'</div><div class="lbl">'+T('dashStatTopGpa')+' &middot; '+topGpa.value+' '+T('times')+'</div><div class="stat-hint">'+T('statHintFilter')+'</div></div>'
  ...
  html += chartCardHtml(T('dashByGpa'), byGpa, 'var(--teal)', null, trGpa, 'fGpa');
...
function goToFilteredSearch(filterKey, filterValue) {{
  document.getElementById('q').value = '';
  document.getElementById('fGpa').value = '';
  document.getElementById('fCat').value = '';
  document.getElementById('fYear').value = '';
...
function wireDashClicks(root) {{
  var clickable = root.querySelectorAll('[data-filter-key], [data-reset-filters]');
  for (var i=0;i<clickable.length;i++) {{
    (function(el){{
      el.addEventListener('click', function(){{
        ...
        else if (key && val) {{ goToFilteredSearch(key, val); }}
      }});
    }})(clickable[i]);
  }}
}}
