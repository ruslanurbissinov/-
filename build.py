...
var I18N = {{
  ru: {{
    ...
    tabInvestigation:'Акт расследования', tabWorkOrder:'Акт выполненных работ', tabVenting:'Акт стравливания',
    ...
    noVentingAct:'Отдельный акт стравливания по этому случаю не оформлялся — данные об объёме стравленного газа не зафиксированы.', openVentingAct:'Открыть акт стравливания',
    exactTime:'Время:',
  ...
  en: {{
    ...
    tabInvestigation:'Investigation report', tabWorkOrder:'Completion report', tabVenting:'Gas venting report',
    ...
    noVentingAct:'No separate gas venting report was filed for this case — vented volume data is not recorded.', openVentingAct:'Open venting report',
    exactTime:'Time:',
...
function buildCard(inc, tokens, scoreBadge) {{
  ...
  var hasVenting = !!(vent && (vent.volume_m3 !== undefined && vent.volume_m3 !== null || vent.source));
  ...
  if (!hasVenting) {{
    ...
    if (vent.source) {{
      htmlStr += '<p><b>'+T('docs')+'</b> <a class="doc-link" href="'+encodeURIComponent(vent.source)+'" target="_blank">'+T('openVentingAct')+'</a></p>';
    }}
  }}
  htmlStr += '</div>';
