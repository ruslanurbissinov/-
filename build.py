...
<style>
  ...
  *{{box-sizing:border-box;}}
  ...
  .doc-src{{color:var(--text-muted);font-weight:600;cursor:default;border-bottom:1px dotted var(--border);}}
  ...
  .doc-badge{{display:inline-block;background:var(--surface);border:1px solid var(--border);color:var(--navy);font-size:12px;font-weight:600;padding:3px 10px;border-radius:6px;margin:0 6px 6px 0;cursor:default;}}
...
var I18N = {{
  ru: {{
    ...
    docs:'Документы:', openInvestigationAct:'Акт расследования',
    openWorkAct:'Акт выполненных работ', openDefectAct:'Дефектный акт',
  ...
  en: {{
    ...
    docs:'Documents:', openInvestigationAct:'Investigation report',
    openWorkAct:'Completion report', openDefectAct:'Defect report',
...
function buildCard(inc, tokens, scoreBadge) {{
  ...
  var docLinks = [];
  if (inc.source) {{ docLinks.push('<span class="doc-src" title="'+escapeHtml(inc.source)+'">'+T('openInvestigationAct')+'</span>'); }}
  if (inc.remediation && inc.remediation.length) {{
    for (var w2=0;w2<inc.remediation.length;w2++) {{
      var wk2 = inc.remediation[w2];
      if (wk2.work_source) {{
        docLinks.push('<span class="doc-src" title="'+escapeHtml(wk2.work_source)+'">'+T('openWorkAct')+'</span>');
      }}
    }}
  }}
  if (inc.defect_source) {{ docLinks.push('<span class="doc-src" title="'+escapeHtml(inc.defect_source)+'">'+T('openDefectAct')+'</span>'); }}
  if (inc.work_source) {{ docLinks.push('<span class="doc-src" title="'+escapeHtml(inc.work_source)+'">'+T('openWorkAct')+'</span>'); }}
...
function buildAIAnalysis(scored) {{
  ...
  if (docList.length) {{
    ...
    for (var d=0;d<docList.length;d++) {{
      var di = docList[d];
      if (di.source) {{
        html += '<span class="doc-badge" title="'+escapeHtml(di.source)+'">'+escapeHtml(di.act)+'</span>';
      }} else {{
